...
export default function Footer() {
  ...
  return (
    <footer className="w-full relative bg-slate-950 text-white pt-24 pb-12 overflow-hidden border-t border-white/10">
      ...
      {/* 2. High-Contrast Glowing Orbs (Replaced w-125 with standard rem to avoid hydration issues) */}
      <div className="absolute top-0 right-0 w-120 h-120 bg-indigo-600/10 rounded-full blur-[120px] -mr-40 -mt-40 animate-pulse" />
      <div className="absolute bottom-0 left-0 w-100 h-100 bg-blue-600/10 rounded-full blur-[100px] -ml-40 -mb-40" />
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-16 mb-20">
            ...
            <div className="flex items-center gap-4">
              {[
                ...
                <a 
                  ...
                  href={social.href} 
                  target="_blank" 
                  className="p-4 bg-white/5 border border-white/10 rounded-2xl text-slate-300 hover:text-white hover:bg-indigo-600 hover:border-indigo-500 transition-all duration-300 shadow-xl"
                >
                  <social.icon size={20} />
                </a>
              ))}
            ...
            <div className="grid grid-cols-2 gap-y-5 gap-x-12">
              {navLinks.map((link) => (
                <a 
                  key={link.name} 
                  href={link.href}
                  className="text-slate-300 hover:text-white text-base font-bold transition-all flex items-center gap-3 group"
                >
          ...
          <div className="md:col-span-3">
            ...
            <div className="bg-linear-to-br from-indigo-600 to-blue-700 p-8 rounded-[2.5rem] shadow-2xl shadow-indigo-500/20 ring-1 ring-white/20">
              <div className="flex items-center gap-3 mb-4">
                <div className="flex h-3 w-3 relative">
                  <span className="animate-ping absolute h-full w-full rounded-full bg-green-300 opacity-75"></span>
                  <span className="relative h-3 w-3 rounded-full bg-green-400"></span>
                </div>
        ...
        <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="flex flex-col md:flex-row items-center gap-6 text-slate-500 text-sm font-bold">
            <span className="text-slate-200">© {year} Krishnendu Ghosal</span>
