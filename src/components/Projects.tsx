    ...
    <section id="projects" className="py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
        {projects.map((project, i) => (
          <div 
            key={i} 
            className="group relative bg-white border border-slate-200 rounded-4xl p-8 hover:border-indigo-500 transition-all duration-300 hover:shadow-2xl hover:shadow-indigo-50/50"
          >
            <div className="flex justify-between items-start mb-6">
              <div className="p-3 bg-slate-50 rounded-2xl group-hover:bg-indigo-50 transition-colors">
                <Code2 className="w-6 h-6 text-slate-400 group-hover:text-indigo-600" />
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {project.tech.map((t) => (
                <span key={t} className="text-[10px] uppercase tracking-wider font-bold text-slate-400 border border-slate-100 px-2 py-1 rounded-md">
                  {t}
                </span>
              ))}
            </div>
      ...
      <div className="relative bg-slate-900 rounded-[2.5rem] p-12 md:p-20 text-center overflow-hidden shadow-2xl">
        ...
        <div className="absolute inset-0 z-0">
          <video 
            ...
            muted 
            playsInline 
            className="w-full h-full object-cover backdrop-blur-sm opacity-30"
          >
        ...
        <div className="relative z-10">
          ...
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-4 bg-white text-slate-900 hover:bg-indigo-600 hover:text-white px-10 py-5 rounded-2xl font-black transition-all duration-300 group shadow-xl"
          >
